...
export default function BannerForm({ banner }: { banner?: Banner } = {}) {
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      fd.append("file", file); fd.append("folder", "temple/banners");
      const res = await fetch("/api/upload", { method: "POST", body: fd });
      const data = await res.json();
      if (!res.ok) { setError(data.error || "Upload failed."); setPreviewSrc(banner?.image ?? ""); }
      else setUploadedUrl(data.url);
    } catch { setError("Upload failed."); setPreviewSrc(banner?.image ?? ""); }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) { const d = await res.json(); setError(d.error || "Failed to save banner."); }
  ...
  return (
    <>
      {isEdit ? (
        <button onClick={() => setOpen(true)} title="Edit banner" className="p-1.5 bg-amber-100 text-amber-700 rounded hover:bg-amber-200 transition-colors">
          ...
          <div className="bg-white rounded-2xl shadow-xl gold-border w-full max-w-lg flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
              <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
                ...
                <div>
                  <label className={labelClass}>
                    Slide Image {!isEdit && "*"}
                    {isEdit && <span className="text-foreground/40 font-normal"> (click to replace)</span>}
                  </label>
                  <div
                  ...
                  >
                    {previewSrc ? (
                      <>
                        ...
                        {!uploading && (
                          <button type="button" onClick={(ev) => { ev.stopPropagation(); setPreviewSrc(banner?.image ?? ""); setUploadedUrl(banner?.image ?? ""); if (fileRef.current) fileRef.current.value = ""; }}
                            className="absolute top-2 right-2 bg-black/60 hover:bg-black/80 text-white rounded-full p-1 transition-colors" title={isEdit ? "Reset to original" : "Remove image"}>
                            <X className="w-4 h-4" />
                          </button>
                        )}
                      </>
                ...
                <div>
                  <label className={labelClass}>Slide Title <span className="text-foreground/40 font-normal">(optional)</span></label>
                  <input value={form.title} onChange={(e) => setForm((p) => ({ ...p, title: e.target.value }))} className={inputClass} placeholder="Defaults to temple name" />
                </div>
                <div>
                  <label className={labelClass}>Subtitle <span className="text-foreground/40 font-normal">(optional)</span></label>
                  <input value={form.subtitle} onChange={(e) => setForm((p) => ({ ...p, subtitle: e.target.value }))} className={inputClass} placeholder="Defaults to temple tagline" />
                </div>
                <div>
                  <label className={labelClass}>Description <span className="text-foreground/40 font-normal">(optional)</span></label>
                  <textarea value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} rows={3} className={inputClass} placeholder="Defaults to temple mission statement" />
                </div>
                <div>
                  <label className={labelClass}>Primary Button <span className="text-foreground/40 font-normal">(optional — defaults to &ldquo;Book a Service&rdquo;)</span></label>
                  <div className="grid grid-cols-2 gap-3">
                    <input value={form.ctaText} onChange={(e) => setForm((p) => ({ ...p, ctaText: e.target.value }))} className={inputClass} placeholder="Button text" />
                    <input value={form.ctaLink} onChange={(e) => setForm((p) => ({ ...p, ctaLink: e.target.value }))} className={inputClass} placeholder="/services" />
                  </div>
                </div>
                <div>
                  <label className={labelClass}>Secondary Button <span className="text-foreground/40 font-normal">(optional — defaults to &ldquo;Donate Now&rdquo;)</span></label>
                  <div className="grid grid-cols-2 gap-3">
                    <input value={form.cta2Text} onChange={(e) => setForm((p) => ({ ...p, cta2Text: e.target.value }))} className={inputClass} placeholder="Button text" />
                    <input value={form.cta2Link} onChange={(e) => setForm((p) => ({ ...p, cta2Link: e.target.value }))} className={inputClass} placeholder="/donate" />
                  </div>
                </div>
              ...
              <div className="px-6 py-4 border-t border-gold/15 flex gap-3 shrink-0">
                <button type="submit" disabled={submitting || uploading || !uploadedUrl} className="btn-primary flex-1 flex items-center justify-center gap-2 py-2.5 disabled:opacity-50 disabled:cursor-not-allowed">
                  {submitting ? <><Loader2 className="w-4 h-4 animate-spin" /> Saving…</>
                    : uploading ? <><Upload className="w-4 h-4 animate-pulse" /> Uploading…</>
                    : isEdit ? "Save Changes" : "Save Banner"}
                </button>
